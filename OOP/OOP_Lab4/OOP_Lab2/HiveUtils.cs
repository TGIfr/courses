...
namespace OOP_Lab3
{
    class HiveUtils
    {
        ...
        public static void SerializeApiaryBinary(Apiary ap) {
            BinaryFormatter formatter = new BinaryFormatter();
            using (FileStream fs = new FileStream("ap.dat", FileMode.Create)) {
                formatter.Serialize(fs, ap);
                Console.WriteLine("Serialized");
            }
        }
        ...
        public static Apiary DeserializeApiaryBinary() {
            BinaryFormatter formatter = new BinaryFormatter();
            using (FileStream fs = new FileStream("ap.dat", FileMode.OpenOrCreate)) {
                return (Apiary)formatter.Deserialize(fs);
            }
        }
